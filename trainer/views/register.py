...
from core.auth import CustomManagement
from core.models import CustomUser
from trainer.forms import TrainerForm, TrainerFormRegister, Trainer
...
class TrainerRegister(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        return render(request, self.template_name,
                      {'form_trainer': self.form_trainer, 'form_trainer_user': self.form_trainer_user})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.is_ajax and request.method == 'POST':
            ...
            if form_trainer_user.is_valid():
                ...
                manageGroup.set_group_to_user(group='trainer', user=request.POST.get('username'), app_label='core',
                                              model='CustomUser')
                trainer = Trainer()
                ...
                trainer.details = CustomUser.objects.get(username=request.POST.get('username'),
            ...
            return JsonResponse({'status': 'error', 'reason': [v[0] for k, v in form_trainer_user.errors.items()]},
                                safe=False,
