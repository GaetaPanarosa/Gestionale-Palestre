...
class PermissionTests(TestCase):
    ...
    def create_permission(self, permission, app_label, model):
        try:
            for i in permission:
                Permission.objects.get_or_create(codename=str(i[0]), name=str(i[1]), content_type=ContentType.objects.get(app_label=str(app_label), model=str(model)))
...
########################## TEST DEI METODI ###############################################
    ...
    def testCreatePermission(self):
        self.assertTrue(self.create_permission(permission=self.admin_permission_list,app_label='core',model='CustomUser'))
        self.assertTrue(self.create_permission(permission=self.user_permission_list,app_label='core',model='CustomUser'))
    ...
    def testSetGroupToUser(self):
        ...
        self.assertTrue(self.create_permission(permission=self.user_permission_list,app_label='core',model='CustomUser'))
        self.assertTrue(self.set_permission_to_group('index_admin', 'amministratore'))
        self.assertTrue(self.set_group_to_user(group='amministratore',user='g.panarosa',app_label='core',model='CustomUser'))