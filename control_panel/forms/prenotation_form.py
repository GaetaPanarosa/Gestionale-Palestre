...
class ModPrenotationForm(forms.ModelForm):
    ...
    time = forms.ModelChoiceField(queryset=Hours.objects.filter(active=True).order_by('start_hour'), widget=forms.Select(
        attrs={'class': 'form-control select2-single select2-hidden-accessible', 'data-width': '100%',
               'form': 'mod_prenotation', 'data-placeholder': 'Seleziona fascia oraria', 'readonly': 'readonly'}))
