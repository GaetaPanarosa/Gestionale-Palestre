...
from control_panel.forms.hours_form import HoursForm
...
class HoursManagement(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.is_ajax and request.method == 'POST':
            ...
            if form_hours.is_valid():
                hours = Hours.objects.filter(start_hour = form_hours.cleaned_data['start_hour'], end_hour=form_hours.cleaned_data['end_hour'])
                if not hours.exists():
                    if form_hours.cleaned_data['start_hour'] > form_hours.cleaned_data['end_hour']:
                        response = {'status': '500', 'reason': 'L\'orario di inizio non può essere successivo all\'orario di fine.'}
    ...
    def info(request,pk):
        ...
        response = {'status':'ok','start_hour':hours_info.start_hour, 'end_hour':hours_info.end_hour,'pk':hours_info.pk}
        return JsonResponse(response, safe=False, content_type='application/json')
